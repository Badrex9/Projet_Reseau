...
#define LENGTH_RAW 3000
//Longueur du buffer contenant le fichier ouvert
#define LENGTH_FILE 3000000  
...
void ouverture_et_lecture_fichier(char path[BUFFER_LEN], int socket){
	...
	char* envoie = malloc(LENGTH_FILE*sizeof(char));
	bzero(envoie, LENGTH_FILE);
	printf("On est là\n");
    if (fichier != NULL)
    {
	...
	write(socket, envoie, strlen(envoie));
	free(envoie);
}