...
// Taille du buffer final de retour
...
void manageClient(int clients, char* port2, struct sockaddr_in adresse2) {
    ...
    while ((clientSocket = accept(clients, (struct sockaddr *) &clientAdresse, &addrLen)) != -1){
        ...
        int ret = write(serverSocket2, path, BUFFER_LEN);
        int valread;
        
        char* buffer_final = malloc(BUFFER_LEN_FINAL*sizeof(char));
        //Lecture du fichier
        bzero(buffer_final, BUFFER_LEN_FINAL);

        valread = read(serverSocket2, buffer_final, BUFFER_LEN_FINAL);

        //printf("Valeur Buf: %s\n", buffer_final);

        close(serverSocket2);
        write(clientSocket,buffer_final, BUFFER_LEN_FINAL);
